...
WaterLevelSensor::WaterLevelSensor(unsigned int sensorNum) noexcept
	: SensorWithPort(sensorNum, "Water Level"), lowTemp(DefaultLowTemp), highTemp(DefaultHighTemp), filtered(true), adcFilterChannel(-1)
{
	CalcDerivedParameters();
}

GCodeResult WaterLevelSensor::Configure(GCodeBuffer& gb, const StringRef& reply, bool& changed)
{
...
GCodeResult WaterLevelSensor::Configure(const CanMessageGenericParser& parser, const StringRef& reply) noexcept
{
	bool seen = false;
...
void WaterLevelSensor::Poll() noexcept
{
	if (filtered && adcFilterChannel >= 0)
	{
		...
		if (tempFilter.IsValid())
		{
			...
			SetResult((averagedTempReading * WaterIncreasePerCount) + lowTemp, TemperatureError::ok);
		}
		else
		{
	...
	{
		SetResult((port.ReadAnalog() * WaterIncreasePerCount) + lowTemp, TemperatureError::ok);
	}
}

void WaterLevelSensor::CalcDerivedParameters() noexcept
{
	...
	WaterIncreasePerCount = (highTemp - lowTemp)/((filtered) ? FilteredAdcRange : UnfilteredAdcRange);
}
